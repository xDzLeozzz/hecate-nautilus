...
# --- 1) entrypoint.sh (tolerante a Windows bind mount)
...
# chown pode falhar em volumes Windows: não abortar
...
server:
  ...
  # rede docker interna / exposição via ports
...
# PS 5.1 não tem ternário - usa if/else
$hostIP = "127.0.0.1"
...
# --- 6) Firewall IN & OUT (sem ternário)
...
docker rm -f hecate-unbound 2>$null | Out-Null
# Remover volumes antigos possíveis
docker volume rm hecate-nautilus_unbound_etc 2>$null | Out-Null
docker volume rm unbound_etc 2>$null | Out-Null
...
if(-not $ok){ Write-Warning "Container nao ficou healthy a tempo (Health='$health')." }
...
# --- 11) Testes no host (UDP/TCP em 127.0.0.1 e pelo IP do host)
...
# Descobrir um IPv4 do host (não loopback, não vEthernet)
...
if($ipWorks -and ($loopbackUdpFail -or $loopbackTcpFail)){
  Write-Warning "Loopback (127.0.0.1) ta bloqueando/limitando. Use -BindAll (porta em 0.0.0.0) ou consulte $hostIPv4:5335."
}elseif($loopbackUdpFail -and -not $ipWorks){
  Write-Warning "UDP do host nao chegou no container. Verifique Antivirus/Firewall/stack Winsock. Consulte o relatorio."
}elseif(-not $ok){
