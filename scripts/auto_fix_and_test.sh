...
REPO_URL="https://github.com/xDzLeozzz/hecate-nautilus.git"
REPO_DIR="$HOME/hecate-nautilus"
cd "$REPO_DIR" 2>/dev/null || { echo "Repo não encontrado"; exit 1; }

log "[*] Sync com origin/main"
git rev-parse --is-inside-work-tree >/dev/null 2>&1 || { echo "não é repo"; exit 1; }
git checkout -q main || true
git fetch origin || true
git merge -X ours --no-edit origin/main || true

log "[*] Dockerfile (sem chown em mounts RO; --with-ssl; anchor refresh no start)"
...
RUN printf '%s\n' '#!/bin/sh' 'set -e' \
  ...
  'echo "Refreshing root trust anchor (RFC5011) ..."; unbound-anchor -a /var/lib/unbound/root.key -R -v || true' \
...
log "[*] unbound.conf (validator estrito; sem forwarders; logs em stderr)"
...
# commit/push
git add -A
git commit -m "fix: dnssec estrito + anchor refresh; sem forwarders" || true
...
echo "== logs recentes =="
docker logs --since 3m hecate-unbound | tail -n 80 || true
